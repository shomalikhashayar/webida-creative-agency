<div class="column items-center q-mt-xl q-mb-lg">
    <img style="width: 50%" src="<?php echo get_template_directory_uri(); ?>/assets/images/nothing-found.svg" alt="">
    <div class="message q-px-xl q-mt-md text-center">
        <p class="text-h6 text-secondary text-weight-700 no-letter-spacing">من خیلی متاسفم ولی چیزی که دنبالشی اینجا
            نیست</p>
        <p class="text-body2 no-letter-spacing text-dark line-height-xs">شاید باید دوباره تلاش کنی و یا شاید بهتره چیز دیگه ای رو
            جستجو کنی</p>
    </div>
</div>
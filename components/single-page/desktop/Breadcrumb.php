<div class="gt-sm">
    ...
    if (is_single()) {
        ...
        if ($categories) {
            ...
            echo $before . '<a href="' . get_category_link($category->term_id) . '" class="text-primary no-decoration items-center">' . '<q-icon name="o_category" class="q-mr-xs" size="20px" color="primary"></q-icon>' . $category->name . "</a>" . $after . $delimiter;
        }

        // Display an icon and wrap the post title in a <span> with items-center class
        echo $before . '<q-icon name="o_article" class="q-mr-xs" size="20px" color="secondary"></q-icon><span class="text-secondary items-center">' . get_the_title() . '</span>' . $after;
    }

<div class="gt-sm">
    ...
    $before = '<span class="text-body2 current">'; // Tag before the current page
    ...
    echo $before . '<a href="' . $homeLink . '/blog/" class="text-primary no-decoration items-center">' . '<q-icon name="o_local_library" class="q-mr-xs" size="sm" color="primary"></q-icon>' . 'بلاگ' . "</a>" . $after . $delimiter;
    ...
    if (is_single()) {
        ...
        if ($categories) {
            ...
            echo $before . '<a href="' . get_category_link($category->term_id) . '" class="text-primary no-decoration items-center">' . '<q-icon name="o_category" class="q-mr-xs" size="sm" color="primary"></q-icon>' . $category->name . "</a>" . $after . $delimiter;
        }
